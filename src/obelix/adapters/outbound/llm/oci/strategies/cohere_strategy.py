...
from obelix.adapters.outbound.llm.oci.strategies.base_strategy import OCIRequestStrategy
...
class CohereRequestStrategy(OCIRequestStrategy):
    ...
    def convert_tools(self, tools: list[Tool]) -> list[CohereTool]:
        """
        Convert Tool objects to CohereTool format.
        """
        if not tools:
