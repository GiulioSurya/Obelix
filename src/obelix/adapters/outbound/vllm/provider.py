...
class VLLMProvider(AbstractLLMProvider):
    ...
    async def invoke(
        ...
        for attempt in range(1, self.MAX_EXTRACTION_RETRIES + 1):
            logger.debug(
                f"vLLM invoke: model={self.model_id}, messages={len(working_messages)}, tools={len(converted_tools)}, attempt={attempt}"
            )
            ...
            try:
                return self._convert_response_to_assistant_message(
                    outputs[0], has_tools=bool(converted_tools)
                )
    ...
    def _convert_messages(self, messages: list[StandardMessage]) -> list[dict]:
        ...
        logger.debug(
            f"Converted {len(messages)} messages to {len(converted)} vLLM messages"
        )
        return converted
