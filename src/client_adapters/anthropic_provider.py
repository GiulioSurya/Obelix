...
from src.obelix_types import SystemMessage, StandardMessage, AssistantMessage, HumanMessage, ToolMessage
...
class AnthropicProvider(AbstractLLMProvider):
    ...
    Supports:
    - System obelix_types (as separate parameter, not in obelix_types array)
    ...
    async def invoke(self, messages: List[StandardMessage], tools: List[ToolBase]) -> AssistantMessage:
        """
        Call the Anthropic model with standardized obelix_types and tools.
        ...
        for attempt in range(1, self.MAX_EXTRACTION_RETRIES + 1):
            logger.debug(f"Anthropic invoke: model={self.model_id}, obelix_types={len(working_messages)}, tools={len(converted_tools)}, attempt={attempt}, thinking_mode={self.thinking_mode}")
            ...
            api_params: Dict[str, Any] = {
                ...
                "obelix_types": conversation_messages,
    ...
    def _convert_messages(self, messages: List[StandardMessage]) -> Tuple[Optional[str], List[dict]]:
        """
        Convert standardized obelix_types to Anthropic format.

        Anthropic requires system message as separate parameter, not in obelix_types array.
        ...
        logger.debug(f"Converted {len(messages)} obelix_types: system={'present' if system_content else 'absent'}, conversation={len(conversation_messages)}")
